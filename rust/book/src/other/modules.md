...
| `edomata-testkit` | `munit` | `TestCommand`, `EdomatonAssertions`, `StomatonAssertions` | native, wasm32 |
| `edomata-saas` | `saas` | tenancy, `AuthPolicy`, guards, guarded DSLs, `SaaSPGSchema` (RLS) | native, wasm32 |
...
Every crate is `#![forbid(unsafe_code)]`, documents every public item, and supports Rust 1.88 or later (edition 2024). None of them depends on a JVM, Scala or Java artifact.
